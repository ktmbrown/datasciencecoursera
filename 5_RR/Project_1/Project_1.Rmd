...
# Step 3:
activity_imp <- as.data.frame(activity_narm)
num_nas <- sum(is.na(activity_imp$steps))
c("Number of NA Values" = num_nas)

# Step 4:
per_int_narm <- aggregate(activity_narm["steps"], by=activity_narm["interval"], sum)

ggplot() + aes(per_int_narm$steps) + geom_histogram(binwidth=500, colour=shades[6], fill=shades[5]) +
      geom_vline(xintercept=mean(per_int_narm$steps), lwd=1, linetype=2, color=shades[8]) +
      geom_label(x=13920, y=8.65, aes(fontface=2),color=shades[7], 
                label = paste('Mean:',format(round(mean(per_int_narm$steps), 2), nsmall = 2))) +
      geom_label(x=14140, y=7.7, aes(fontface=2),color=shades[7], 
                 label = paste('Median:',format(round(median(per_int_narm$steps), 2), nsmall = 2))) +
      xlab("Steps per Day") +
      ylab("") +
      ggtitle("Steps Taken per Day between 10/01/2012 to 11/30/2012")

# Step 5:
step_mean_narm <- round(mean(per_int_narm$steps, na.rm=TRUE), 2)
step_median_narm <- round(median(per_int_narm$steps, na.rm=TRUE), 2)
r1<- c("Mean"= step_mean, "Median" = step_median)
r2<- c("Mean" = step_mean_narm, "Median" = step_median_narm)
df_mm <- rbind(r1,r2)
rownames(df_mm) <- c('Before Mean Imputation', 'After Mean Imputation')
knitr::kable(df_mm, caption = "Before and After Mean Imputation")
```
...
activity_narm$dayType <- activity_narm$date %>% sapply(isWeekend) 
knitr::kable(head(activity_narm),caption = 'New variable "dayType" added to dataset' )

# Step 2:
per_int_narm <- aggregate(activity_narm["steps"], by=activity_narm["interval"], sum)
ggplot(data=per_int_narm, aes(x=interval, y=steps)) +
  geom_line()
